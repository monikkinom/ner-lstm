...
class WordVec:
	...
	def train(self, dim, batchSize, epoch):
		...
		with tf.Session(graph = graph) as sess:
			...
			total = (epoch * len(self.corpus.data) * 2) // batchSize
			lossAvg = 0
			for i in range(total):
				...
				if (i + 1) % 100 == 0:
					print str(i + 1) + '/' + str(total), 'Avg Batch Loss:', lossAvg / 100
					lossAvg = 0
			tembed = embeddings.eval()
			nembed = normalizedEmbeddings.eval()
		np.save('TrigramVectors(C=5,normalized).npy', nembed)
		np.save('TrigramVectors(C=5).npy', tembed)
